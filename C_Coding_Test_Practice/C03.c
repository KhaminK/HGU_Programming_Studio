...
void printCountryInfo(struct st_person* p[], int size, int index);
...
int main(){
    ...
    for(int j = 0; j < 6; j++){
        printCountryInfo(pdata, count, j);
    }
...
void printCountryInfo(struct st_person* p[], int size, int index){
    ...
    for (int i = 0; i < size; i++){
        if (p[i]->country == index){
            if (p[i]->gender == 1){
                maleCount++;
            }else{
                femaleCount++;
            }
            totalAge += 2025-p[i]->birthyear;
            count++;
        }
    }
    printf("[%s] Female:%d Male:%d average age:%.1f\n", country_name[index], femaleCount, maleCount, totalAge/count);
}